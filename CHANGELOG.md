...
Version code 63. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.

### Features

- feat(insight): capture weather at activity time/location (GPS-first, falls back to your Profile home location) and factor it into per-activity AI insight scoring

## v2.0.9 — 2026-07-23

Version code 62. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.

### Bug fixes

- fix(dailies): backdate missed-daily completions to the recap date instead of always logging them as done today
- fix(insight): factor in cardio activity metrics (distance/pace/HR/elevation) when generating per-activity AI insights

## v2.0.8 — 2026-07-22

Version code 61. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.

### Features

- feat(exercises): add Planche Push-Up and Weighted Planche Push-Up

### Bug fixes

- fix(exercises): register Planche Push-Up/Weighted Planche Push-Up in autocomplete DB

## v2.0.7 — 2026-07-22

Version code 60. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.
...
- fix(portal-leaks): plural muscle tags, cap growth at maxHp, admin damage-all tool
- fix(web): explicit Log button for score check-ins, morning popup fixes

## v2.0.6 — 2026-07-16

Version code 59. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.
...
Version code 56. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.
...
- fix(substances): invalidate cache + tag auto-linked rows with source
- fix(combat): audit fixes C1+C2+C7 + multi-target leak damage (v2.0.2)
- fix(mobile): morning-recap timing + backdrop dismiss + Today +New Daily placement
- fix(notifications): move shield-digest dedup off the deletable Notification row

### Polish

- refactor(checkins): replace legacy BICEP weekly metric with BICEP_FLEXED + BICEP_RELAXED (v2.0.0)

## v2.0.2 — 2026-07-13

Version code 55. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.
...
Version code 54. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.
...
- fix(mobile): morning-recap timing + backdrop dismiss + Today +New Daily placement
- fix(notifications): move shield-digest dedup off the deletable Notification row
- fix(modal): escape stacking context + nutrition Y-axis room for 4-digit ticks
...
Version code 53. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.
...
Version code 52. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.
...
Version code 51. Tracks the parent
fitquest repo (web + api). Run from fitquest-android after the parent
repo has shipped changes.
